(() => {
  ...
  const Btn = (label, cls, onClick, id) =>
    h("button", { id, class: `btn ${cls || ""}`.trim(), onclick: onClick, text: label });
  ...
  const st = {
    ...
    logText: new State(""),
    toast: new State(null), // { text, kind }
  };

  function setToast(text, kind) {
    st.toast.set({ text, kind: kind || "info" });
    setTimeout(() => {
